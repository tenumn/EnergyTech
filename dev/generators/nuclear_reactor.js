    ...
    registerModule:function(id,state,type,durability){
        if(!this.isModule(id)){
            ...
            if(tile){
                ...
                MachineRegistry.registerPrototype(id,{
                    defaultValues:{
                        durability:durability
                    },
...
MachineRegistry.setDrop("nuclearReactor",BlockID.machineCasing,1);
...
MachineRegistry.registerGenerator(BlockID.nuclearReactor,{
    ...
    tick:function(){
        ...
        for(let x = 0;x <= range;x++){for(let y = 0;y <= range;y++){for(let z = 0;z <= range;z++){
            ...
            var reactor = ReactorRegistry.modules[block.id];
            if(ReactorRegistry.isModule(block.id) && tile){
...
MachineRegistry.registerPrototype(BlockID.fuelRodUranium,{
...
ReactorRegistry.registerModule(BlockID.fuelRodUranium,function(id,data,coords){
    let heat = 3,fuel = 1;
    for(let index in directions){
        var dir = directions[index],block = World.getBlock(coords.x + dir[0],coords.y + dir[1],coords.z + dir[2]),type = ReactorRegistry.getModuleType(block.id);
        if(type == "FuelRod"){
            heat += 3;
            fuel += 1;
        }
...
ReactorRegistry.registerModule(BlockID.heatSink,function(id,data,coords){
...
Block.createBlock("CoolantWater",[
    {name:"Coolant(Water)",texture:[["coolant_bottom",0],["coolant_top",0],["coolant_water",10]],inCreative:true}
],"opaque");
...
MachineRegistry.registerPrototype(BlockID.CoolantWater,{
    defaultValues:{durability:16384},
    tick:function(){this.renderer();},
    renderer:function(){TileRenderer.mapAtCoords(this.x,this.y,this.z,this.id,Math.round(this.data.durability / this.defaultValues.durability * 10));}
});

ReactorRegistry.registerModule(BlockID.CoolantWater,function(id,data,coords){
    let coolant = 3;
    for(let index in directions){
        var dir = directions[index],block = World.getBlock(coords.x + dir[0],coords.y + dir[1],coords.z + dir[2]),type = ReactorRegistry.getModuleType(block.id);
        if(type == "Coolant"){
...
Translation.addTranslation("Coolant(Water)   ",{zh_CN:"冷却器(水)"});
