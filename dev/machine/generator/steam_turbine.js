...
TileRenderer.registerRotationModel(BlockID.steamTurbine,4,[["machine_bottom",1],["machine_top",1],["machine_side",1],["steam_turbine",0],["machine_side",1],["machine_side",1]]);
TileRenderer.registerRotationModel(BlockID.steamTurbine,8,[["machine_bottom",1],["machine_top",1],["machine_side",1],["steam_turbine",1],["machine_side",1],["machine_side",1]]);

Machine.setDrop("steamTurbine",BlockID.machineCasing,1);
...
Machine.registerEUGenerator(BlockID.steamTurbine,{
    ...
    init:function(){
		this.liquidStorage.setLimit("steam",8);
		this.renderer();
    },
    ...
    tick:function(){
        ...
        if(empty && empty.liquid == "steam"){
            ...
            if(this.liquidStorage.getAmount("steam") + storage <= 8 && (liquid2.id == empty.id && liquid2.data == empty.data && liquid2.count < Item.getMaxStack(empty.id) || liquid2.id == 0)){
        ...
        if(amount) this.liquidStorage.setAmount("steam",parseInt(amount * 1000) / 1000);
	...
	getGuiScreen:function(){return GuiSteamTurbine;}
});
