...
FusionReactor.registerModule(BlockID.coilCopper,function(coords,data,id){
    if(id == BlockID.fusionReactor){
        let heat = 3,fuel = 1;
        for(let side = 0;side < 6;side++){var relative = World.getRelativeCoords(coords.x,coords.y,coords.z,side),block = World.getBlock(relative.x,relative.y,relative.z),type = FusionReactor.getModuleType(block.id);
            if(type == "Coil"){
                heat += 3,fuel += 1;
            }
        }
