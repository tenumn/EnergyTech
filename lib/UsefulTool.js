LIBRARY({name:"UsefulTool",version:4,shared:true,api:"CoreEngine"});
...
var UsefulToolCore = {
    ...
    getReplacedCoords:function(coords){
        var place = coords,block = World.getBlock(coords.x,coords.y,coords.z);
        if(!canTileBeReplaced(block.id,block.data)){
            place = coords.relative,block = World.getBlock(coords.relative.x,coords.relative.y,coords.relative.z);
