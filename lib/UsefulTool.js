...
var LiquidCore = {
    ...
    registerItem:function(liquid,empty,full,storage){
        if(!LiquidRegistry.isExists(liquid)){
            LiquidRegistry.registerLiquid(liquid);
        }
    ...
    getFullItem:function(id,data,liquid){
        for(let i in this.items){
            ...
            if(item.liquid == liquid && (id == parseInt(i) && data > 0 || id == item.empty.id)){
                return {id:parseInt(i),data:0,amount:(data || item.storage) / 1000,storage:item.storage / 1000};
            }
        }

        var full = LiquidRegistry.getFullItem(id,data,liquid);
        if(full){
            return {id:full.id,data:full.data,amount:1,storage:1};
        }
...
Translation.getLangFile = function(dir){
    var lang = Game.getLanguage();

    if(FileTools.isExists(dir)){
        var list = FileTools.GetListOfFiles(dir,"lang");
        for(let i in list){
            if(dir + lang + ".lang" == list[i]){
        ...
        if(FileTools.isExists(dir + "en_US.lang")){
            var text = FileTools.ReadKeyValueFile(dir + "en_US.lang","=");
            for(let n in text){
                var data = {}
                data[lang] = text[n];
                Translation.addTranslation(n,data);
...
IDRegistry.getOreID = function(){
    ...
    for(id in BlockID){
        ...
        if(!tile && id[0] == "o" && id[1] == "r" && id[2] == "e") ores.push(BlockID[id]);
    }
